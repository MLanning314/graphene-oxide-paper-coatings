...
# Figure 4 - Basically Figure 2 but with Stock included
pres_4_fig <- absorption_HF_fig |>
  filter(coating %in% c("DI Water", "Stock", "25% WBBC", "25% WBBC + 0.1% GO", 
                        "0.1% GO")) |>
  mutate(
    time = (as.numeric(time) - 1) * 10,
    coating = dplyr::recode(coating,
...
pres_fig_4 <- ggplot(pres_4_fig, aes(x = time, y = absorption,
                                     color = coating, shape = coating)) +
  geom_line(linewidth = 1) +
  geom_point(size = 4) +
  geom_errorbar(aes(ymin = absorption - rms,
...
ggsave("presentation_figures/pres_fig_4.png", plot = pres_fig_4,
       width = 10, height = 6, units = "in", dpi = 600)
